...
export const en: Record<MoliCardLocaleKey, string> = {
  ...
  autoScrollSentinelHint: 'Simulates IntersectionObserver sentinel intersections for Moli structure-first geometry to load infinite card lists.',
  ...
  denoiseHint: 'Readability + DOMPurify strips ads, nav bars, and footers before markdown conversion.',
...
export const zh: Record<MoliCardLocaleKey, string> = {
  ...
  autoScrollSentinelHint: '为 Moli 结构优先几何模拟 IntersectionObserver 哨兵可见性，自动加载全部无限瀑布流卡片。',
  shareBrowserContext: '共享浏览器上下文（复用登录态）',
  shareBrowserContextHint: '复用 cookie 与 localStorage；取消勾选则每次抓取使用全新隔离上下文。',
  denoise: '启用降噪算法',
  denoiseHint: '使用 Readability + DOMPurify 清洗广告、导航栏与页脚后转为 Markdown。',
